...
int classes_Hash::resize(Generator *hp, int newSz){
    ...
    if(classes_Array::sizeChange(hp,newSz,2))return 1; // 2=exact size, ret 0 for OK
...
ApplClass* classes_Hash::get(Generator *hp, ApplClass *obj){
    ...
    if(i<0 || i>=hp->ZZds.ZZclasses.sz){
	printf("hashing error bucket=%d for size=%d\n",i,hp->ZZds.ZZclasses.sz);
	printf("classes_Hash:get() cannot find the object\n");
	return NULL;
    }
...
int classes_Hash::add(Generator *hp, ApplClass *obj){ // obj = object to load into the hash table
    ...
    if(i<0 || i>=hp->ZZds.ZZclasses.sz){
	printf("hashing error bucket=%d for size=%d\n",i,hp->ZZds.ZZclasses.sz);
	printf("object not added to the classes_Hash\n");
	return 1;
    }
    p=(ApplClass*)(hp->ZZds.ZZclasses.array[i]);
...
ApplClass* classes_Hash::remove(Generator *hp, ApplClass *obj){
    ...
    if(s==prev){
	hp->ZZds.ZZclasses.array[i]=NULL; 
    }
    else if(s==p){
	hp->ZZds.ZZclasses.array[i]=s->ZZds.ZZclasses.next;
